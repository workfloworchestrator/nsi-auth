...
def test_validate_with_valid_cert(cert_client: FlaskClient) -> None:
    ...
    response = cert_client.get("/validate", headers={"X-Forwarded-Tls-Client-Cert": _SINGLE_CERT_PEM_STR})
...
def test_validate_with_cert_chain(cert_client: FlaskClient) -> None:
    ...
    response = cert_client.get("/validate", headers={"X-Forwarded-Tls-Client-Cert": _CHAIN_HEADER_STR})
    assert response.status_code == 200
    assert response.data == b"OK"
    assert response.headers["X-Auth-Method"] == "mTLS"
...
def test_validate_with_corrupted_cert(cert_client: FlaskClient) -> None:
    ...
    response = cert_client.get("/validate", headers={"X-Forwarded-Tls-Client-Cert": bad_pem})
    assert response.status_code == 403


def test_validate_with_garbage_cert(cert_client: FlaskClient) -> None:
    """Completely garbage input returns 403."""
    response = cert_client.get("/validate", headers={"X-Forwarded-Tls-Client-Cert": "not-a-cert"})
    assert response.status_code == 403


def test_validate_with_empty_cert_header(cert_client: FlaskClient) -> None:
    """Empty header returns 403."""
    response = cert_client.get("/validate", headers={"X-Forwarded-Tls-Client-Cert": ""})
    assert response.status_code == 403
