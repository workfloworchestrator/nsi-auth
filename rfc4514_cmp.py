...
#      emailAddress=knewell@internet2.edu,organizationIdentifier=NTRUS\\+MI-801069584,
#      O=University Corporation For Advanced Internet Development,ST=Michigan,C=US"
...
def dn_rfc2253_string_to_rfc4514_name(rfc2253_string: str) -> x509.Name:
    """Convert an RFC2253 DistinguishedName string into a cryptography.x509.Name object.

    Such a string is generated by: openssl x509 -nameopt RFC2253 -in bla.pem -text
...
def dn_tagvalue_string_to_rfc4514_name(tagvalue_string: str) -> x509.Name:
    """Convert a "tag1=value1,tag2=value2" DistinguishedName into a cryptography.x509.Name.

    Tags (symbolic names, not oids) are taken from the set of valid DN tags as defined by
    cryptography, with e.g. "organizationIdentifier" added, see above.
...
# Characters that RFC 4514 section 2.4 requires to be backslash-escaped in a value.
_RFC4514_SPECIAL_CHARS = frozenset(',+"#<>;=\\')
...
_TAG_VALUE_PATTERN = r"([^\s,=]+)=((?:\\.|[^=,\\])*)(?=\s*,?\s*[^\s,=]+=|$)"


def _unescape_value(value: str) -> str:
    r"""Unescape RFC 4514 escape sequences, converting \X back to X."""
    return re.sub(r"\\(.)", r"\1", value, flags=re.DOTALL)


def _escape_rfc4514(value: str) -> str:
    r"""Escape a value per RFC 4514 section 2.4.

    Special chars (, + " # < > ; = \) always get a backslash; a space only when
    it is the leading or trailing character. Embedded spaces are left alone.
    """

    def escape(index: int, char: str) -> str:
        if char in _RFC4514_SPECIAL_CHARS:
            return "\\" + char
        if char == " " and index in (0, len(value) - 1):
            return "\\ "
        return char

    return "".join(escape(index, char) for index, char in enumerate(value))


# Arno talking to confer.to:
def confer_parse_tag_pairs(input_string: str) -> str:
    r"""Normalize a string of tag=value pairs to comma separators, order preserved.

    Separators in the input may be commas, spaces, or both, and values may be
    escaped, unescaped, or a mix; the output is consistently escaped per RFC 4514.
    ...
    matches = re.findall(_TAG_VALUE_PATTERN, input_string, re.DOTALL)
    if not matches:
        return input_string
    ...
    # else _escape_rfc4514 would escape the leading '#' and turn the BER-hex
    # value into a literal string that never decodes.
    return ",".join(
        f"{tag}={value if value.startswith('#') else _escape_rfc4514(_unescape_value(value))}" for tag, value in matches
    )
...
def _strip_ber_tlv(value: str) -> str:
    r"""Strip BER TLV framing from `value` if present, else return unchanged.
...
def name_attrs(name: x509.Name) -> frozenset[tuple[str, str]]:
    ...
    compare equal.
    """
    return frozenset((a.oid.dotted_string, _value_str(a.value)) for rdn in name.rdns for a in rdn)


def subject_dn_from_cert_pem(cert_pem_bytes: bytes) -> x509.Name:
    """Parse Subject DN from a PEM-encoded certificate.

    Args:
        cert_pem_bytes (bytes): PEM framed as per cryptography's rules
    ...
    Raises:
        ValueError: If cert_pem_bytes is not a valid PEM-encoded x.509 certificate.
    """
    return x509.Name(load_pem_x509_certificate(cert_pem_bytes).subject)


def subject_dn_from_pem_header(header_value: str) -> x509.Name:
...
def subject_dn_from_traefik_cert_pem(traefik_cert_str: str) -> x509.Name:
...
def subject_dn_from_traefik_cert_info(header_value: str) -> x509.Name | None:
...
def _xfcc_field(header_value: str, key: str) -> str | None:
    ...
    """
    match = re.search(rf'(?:^|;)\s*{re.escape(key)}=("([^"]*)"|[^;]*)', header_value, re.IGNORECASE)
    if not match:
        return None
    return match.group(2) if match.group(2) is not None else match.group(1)


def subject_dn_from_xfcc_cert(header_value: str) -> x509.Name | None:
...
def subject_dn_from_xfcc_subject(header_value: str) -> x509.Name | None:
